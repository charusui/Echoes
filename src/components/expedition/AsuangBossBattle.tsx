...
import wildSummitLayer_bg from '../../assets/boss_bg/wildSummitLayer_bg.png';

interface AsuangBossBattleProps {
  bossName: string;
  onComplete: (totalDamage: number, totalStagger: number, hits: number) => void;
}

export default function AsuangBossBattle({
  bossName,
  onComplete,
}: AsuangBossBattleProps) {
  ...
  return (
    <div className="relative w-full h-full min-h-screen flex flex-col items-center justify-center overflow-hidden bg-[#0f0c0c]">
      
      {/* ─── BACKGROUND LAYERS ─── */}
      <div 
        className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat" 
        style={{ backgroundImage: `url(${wildSummit_bg})` }} 
      />
      <div 
        className="absolute inset-0 z-10 bg-cover bg-center bg-no-repeat" 
        style={{ backgroundImage: `url(${wildSummitLayer_bg})` }} 
      />

      {/* ─── WINGSLAM MINIGAME UI ─── */}
      <div className="w-full relative z-50 flex flex-col items-center justify-center pointer-events-auto select-none animate-fadeIn">
        ...
        {!isFinished ? (
          ...
          <div className="max-w-md w-full bg-[#facc15] border-[4px] border-[#0f0c0c] p-6 shadow-[8px_8px_0px_0px_#0f0c0c] flex flex-col items-center justify-center gap-3 animate-bounce">
            <span className="font-orbitron font-black text-2xl text-[#0f0c0c] uppercase text-center">
