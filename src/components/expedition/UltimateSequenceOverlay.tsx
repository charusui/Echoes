...
export function UltimateSequenceOverlay({ hero, instrument, onComplete }: UltimateSequenceOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-plum-950/90 flex flex-col animate-in fade-in duration-300">
      
      {phase === 'rhythm' && (
        <div className="relative w-full h-full flex flex-col pt-safe">
          
          {/* Header */}
          <div className="px-6 py-4 flex justify-between items-center bg-plum-900 border-b-[3px] border-ink">
            <div>
              <h2 className="font-bold text-2xl leading-none text-gold-300">
                Overdrive Ultimate
              </h2>
            ...
            <div className="flex items-center gap-6">
              <div className="text-right">
                <p className="text-xs text-parchment-500">Combo</p>
                <p className={` font-bold text-3xl transition-colors duration-200 ${gameState.combo >= 10 ? 'text-gold-300' : 'text-parchment-100'}`}>
                  {gameState.combo}x
                </p>
              </div>
              <div className="h-10 w-[3px] bg-ink" />
              <div className="text-right">
                <p className="text-xs text-parchment-500">Time</p>
                <p className="font-bold text-2xl text-parchment-100">
          ...
          <div className="relative flex-1 bg-plum-900 border-x-[3px] border-ink mx-auto w-full max-w-7xl">
            ...
            {hitIndicator && (
              ...
              >
                {hitIndicator.type === 'perfect' && (
                  <span className="font-bold block text-5xl md:text-7xl text-gold-300 animate-comic-pop flex items-center gap-2">
                    <Zap className="fill-current w-12 h-12" />
                    {hitIndicator.text}
                  </span>
                )}
                {hitIndicator.type === 'good' && (
                  <span className="font-bold block text-4xl md:text-6xl text-xp animate-comic-pop-slight">
                    {hitIndicator.text}
                  </span>
                )}
                {hitIndicator.type === 'Miss' && (
                  <span className="font-bold block text-4xl md:text-6xl text-parchment-500 animate-comic-glitch line-through decoration-hp decoration-[8px]">
                    {hitIndicator.text}
                  </span>
                )}
              </div>
